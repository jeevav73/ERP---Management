  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        console.error('Failed to load agents for login report', err);
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className="bg-white border-b border-slate-100 px-6 py-4 mb-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-black">User Login Report</h2>
                </div>
                <div className="text-sm text-slate-500">Agent login & session details</div>
              </div>
            </div>

            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <UserLoginReport agents={agents} calls={calls} onClose={() => {}} />
            </div>
          </div>
        </div>
      </div>
